...
console.log("Starting wuttodo-taskmanagement-service service...");
...
const start = async () => {
  process.title = "node-wuttodo-taskmanagement-service";
  ...
  try {
    ...
    await setRedisData(
      "wuttodo-taskmanagement-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key wuttodo-taskmanagement-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "wuttodo-taskmanagement-service is listening HTTP/REST port " +
...
/* starting the server wuttodo-taskmanagement-service .... */
start();
