...
const updateTrewytgreById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Trewytgre.findOne({
    ...
    const [rowsCount, [dbDoc]] = await Trewytgre.update(dataClause, options);
    if (!dbDoc) {
...
module.exports = updateTrewytgreById;
