...
const updateRfewtgwreById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Rfewtgwre.findOne({
    ...
    const [rowsCount, [dbDoc]] = await Rfewtgwre.update(dataClause, options);
    if (!dbDoc) {
...
module.exports = updateRfewtgwreById;
