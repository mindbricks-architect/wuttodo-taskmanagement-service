...
const updateNewtasktotestById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Newtasktotest.findOne({
    ...
    const [rowsCount, [dbDoc]] = await Newtasktotest.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateNewtasktotestById;
