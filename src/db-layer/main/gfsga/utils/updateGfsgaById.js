...
const updateGfsgaById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Gfsga.findOne({ where: { id, isActive: true } });
    ...
    const [rowsCount, [dbDoc]] = await Gfsga.update(dataClause, options);
    if (!dbDoc) {
...
module.exports = updateGfsgaById;
