...
const updateYertyeById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Yertye.findOne({ where: { id, isActive: true } });
    ...
    const [rowsCount, [dbDoc]] = await Yertye.update(dataClause, options);
    if (!dbDoc) {
...
module.exports = updateYertyeById;
