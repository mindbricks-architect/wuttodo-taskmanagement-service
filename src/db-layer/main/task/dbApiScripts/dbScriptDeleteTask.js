...
// This script is called from the DeleteTaskManager
...
const dbScriptDeleteTask = async (apiManager) => {
  const whereClause = apiManager.whereClause;

  try {
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Task.update(
      { isActive: false },
      {
        where: whereClause,
        returning: true,
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeleteTask",
      {
...
module.exports = dbScriptDeleteTask;
