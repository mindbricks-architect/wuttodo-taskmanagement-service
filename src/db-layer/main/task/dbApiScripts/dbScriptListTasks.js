...
// This script is called from the ListTasksManager
...
const dbScriptListTasks = async (apiManager) => {
  ...
  try {
    ...
    let taskList = await Task.findAll(options);

    const dbData = { items: [] };
    apiManager.tasks = [];
    if (!taskList || taskList.length == 0) return dbData;

    dbData.items = Array.isArray(taskList) ? taskList : [taskList];
    dbData.items = dbData.items.map((item) => item.getData());

    dbData.totalRowCount = dbData.items.length;
    dbData.pageCount = 1;

    apiManager.tasks = dbData.items;

    return dbData;
  } catch (err) {
    if (err instanceof HttpError) throw err;
    console.log(err);
    throw new HttpServerError("errMsg_dbErrorWhenExecuting_dbScriptListTasks", {
...
module.exports = dbScriptListTasks;
