...
class CreateTaskMcpController extends TaskManagementMcpController {
  constructor(params) {
    super("createTask", "createtask", params);
    this.dataName = "task";
    this.crudType = "create";
  }

  createApiManager() {
    return new CreateTaskManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    name: "createTask",
    description: "Add a new todo task with a required title.",
    parameters: CreateTaskMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new CreateTaskMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return CreateTaskMcpController.getOutputSchema().parse(result);
